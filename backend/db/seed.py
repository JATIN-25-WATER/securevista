...
def seed_database(db: Session) -> None:
    ...
    # ── Users ──────────────────────────────────────────────────────────────
    users = [
        User(username="operator 1", password_hash=hash_password("op123"), role="admin"),
        User(username="operator 2", password_hash=hash_password("op123"), role="operator"),
        User(username="operator 3", password_hash=hash_password("op123"), role="responder"),
    ]
    db.add_all(users)
    db.flush()  # Get IDs before adding cameras

    cameras = [
        Camera(name="Camera 1 (Webcam)", source_uri="0", status="unknown"),
        Camera(name="Camera 2 (Webcam)", source_uri="0", status="unknown"),
        Camera(name="Camera 3 (Webcam)", source_uri="0", status="unknown"),
    ]
    db.add_all(cameras)
    db.flush()
