...
class VideoSource:
    ...
    BLACKOUT_CONSECUTIVE = 30     # require 30 consecutive dark frames to declare blackout
    ...
    def _assess_health(self, frame: np.ndarray) -> SourceState:
        ...
        # Blackout: mean brightness below threshold (require consecutive dark frames to avoid false startup blackout)
        if gray.mean() < self.BLACKOUT_THRESHOLD:
            self._consecutive_blackout += 1
            if self._consecutive_blackout >= self.BLACKOUT_CONSECUTIVE:
                self._consecutive_frozen = 0
